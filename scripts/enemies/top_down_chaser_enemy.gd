...
@export var simple_search_memory_time: float = 4.5
@export var simple_search_scan_time: float = 1.6
@export var simple_search_arrival_distance: float = 30.0
...
func _update_simple_velocity(delta: float) -> void:
	...
	var target_position := _target.global_position if _is_target_visible() else _last_seen_target_position
	...
	match _attack_state:
		AttackState.CHASE:
			var stop_distance := desired_target_separation if _is_target_visible() else simple_search_arrival_distance
...
func _try_start_attack() -> void:
	if _target == null or not _is_target_visible():
		return
	if not simple_ai_enabled and _is_morale_broken():
		return
