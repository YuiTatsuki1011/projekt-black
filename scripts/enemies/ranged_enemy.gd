...
@export var shot_windup_time: float = 0.58
@export var shot_recovery_time: float = 1.18
@export var initial_shot_delay: float = 0.45
...
@export var knockback_strength: float = 98.0
...
func _enter_windup() -> void:
	...
	body_visual.color = Color(0.28, 0.24, 0.16, 1.0)
...
func _fire_locked_shot() -> void:
	...
	body_visual.color = Color(0.50, 0.12, 0.08, 1.0)
...
func _get_direction_to_target() -> Vector2:
	...
	var target_position := _target.global_position + Vector2(0.0, -24.0)
	return (target_position - muzzle.global_position).normalized()
...
func _set_aim_direction(direction: Vector2) -> void:
	...
	gun_root.position = Vector2(facing * 8.0, -26.0)
...
func _is_target_in_range() -> bool:
	...
	return global_position.distance_squared_to(_target.global_position) <= detection_range * detection_range
...
func _update_telegraph_visual() -> void:
	...
	var end_position := to_local(muzzle.global_position + _locked_direction * telegraph_length)
