...
カメラはプレイヤーを画面左へ寄せる強めのズームへ短く遷移し、背景をぼかした全面UIとして、左にプレイヤー装備/ステータス、中央にバックパックを配置します。
開いている間は照準を非表示にします。右1/3の外部インベントリ枠は、将来のアイテムコンテナ側から `open_external_inventory()` を呼んだ時だけ表示します。
