...
`AmmoHud` は `PlayerController.magazine_status_changed` を受け取り、画面左下に銃の簡易アイコン、銃名、現在装填中の弾数/最大装填数、所持マガジン数を表示します。選択中銃火器に対応するマガジンアイコンも並べ、装備中マガジンはHUD用の仮エントリとして一覧に含め、マガジン枠全体をオレンジ色の点滅枠で囲みます。マガジンごとの残弾量は常に青いゲージで表示し、フィールド装填中のマガジンには同じ `MagazineLoadIndicator` を重ねます。リロードと送弾の進捗は、照準周辺の円形インジケータで表示します。
