...
リロード時はグリッド上の `pistol_magazine` を交換します。`pistol_ammo` はWeapon Inspectから薬室へ1発ずつ装填できます。マガジンを装着しても薬室が空なら即座には発射可能にならず、Rキーまたは射撃入力で送弾動作を行います。マガジンへ弾を詰める操作は今後追加します。
...
`AmmoHud` は `PlayerController.magazine_status_changed` を受け取り、選択中銃火器に対応するバックパック内マガジンを小さなアイコンとして表示します。フィールド装填中のマガジンには同じ `MagazineLoadIndicator` を重ねます。現時点では残弾チェック/記憶システムが未実装のため、HUDは実残弾比率を表示します。将来、R長押しチェックと不明表示を実装したら、この表示は「確認済み情報」だけに制限します。
