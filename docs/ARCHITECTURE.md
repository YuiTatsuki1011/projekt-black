...
現在はプレイヤーの `Equipment` ノードに、暫定の銃火器スロット `ranged_weapon` とサブ武器スロット `melee_weapon` があります。

- 銃火器スロット: 初期値は `basic_pistol.tres`。
...
今後、暫定の単一銃火器スロットを1〜4キーで切り替える4つの銃火器スロットへ拡張します。Q/Eキーは、スキルや背面武器など、手持ちの銃火器と同時に扱う補助装備用に予約しています。
