...
`PlayerController` は、Inspect UIから使うために `insert_active_magazine()`、`eject_active_magazine()`、`insert_chamber_round()`、`eject_chamber_round()` を公開しています。これらは装填状態を更新した後、スロット別の装弾状態を保存し、HUDへ `ammo_changed` を送ります。`get_ammo_display_pair()` は実残弾ではなく、確認済みかどうかを考慮した表示用文字列を返します。
...
`InventoryScreen` は、弾薬アイテムをマガジン上へドラッグ&ドロップした時に、通常移動やスタック結合より先にマガジン装填を判定します。装填可能なら1件だけ装填ジョブを作り、`inventory_magazine_load_time_per_round` ごとに1発ずつ `Inventory.load_round_into_magazine()` を呼びます。装填中のマガジンには `MagazineLoadIndicator` を重ね、ジョブ全体の進捗を円形に描画します。インベントリを閉じる、または対象マガジンを右クリックするとジョブをキャンセルします。
...
`AmmoHud` は `PlayerController.magazine_status_changed` を受け取り、選択中銃火器に対応するバックパック内マガジンを小さなアイコンとして表示します。フィールド装填中のマガジンには同じ `MagazineLoadIndicator` を重ねます。未確認マガジンは `?`、確認済みマガジンは実残弾比率を表示します。左上の `AMMO` 表示も `PlayerController.get_ammo_display_pair()` を通し、R長押し確認が完了するまで装備中銃火器の正確な装弾数を隠します。
