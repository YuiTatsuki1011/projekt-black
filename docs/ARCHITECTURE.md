...
- `LootDropper`: 敵の死亡時に `DroppedItem` を生成する共通部品。敵本体は死亡時にこのノードを呼ぶだけにし、何を何個落とすかは敵シーン側の設定に置く。
...
Fキーで調べられる距離にある `Area2D` は、プレイヤー側が一時的な `Line2D` の白枠を付けてハイライトします。複数の対象が近くにある場合は、マウスカーソルが重なっている対象を優先し、重なっていない場合はプレイヤーに最も近い対象を使います。
...
`RangedWeaponData` は `magazine_item_id`、`chamber_size`、`starting_spare_magazines`、`chamber_feed_time` を持ちます。`PlayerController` は銃火器スロットごとに、装着中マガジン内の弾数、薬室内の弾数、装着マガジン種別を保存します。
...
`Inventory` はマガジン装填用の問い合わせと操作APIを持ちます。`get_magazine_entry_info()` は、対象エントリがマガジンかどうか、対応弾薬ID、容量、現在弾数を返します。`load_round_into_magazine()` は、別インベントリを含む弾薬エントリから1発だけ消費し、対象マガジンの `metadata.ammo_count` を1増やします。対応弾薬でない、満タン、弾薬がない場合は失敗します。

`InventoryScreen` は、弾薬アイテムをマガジン上へドラッグ&ドロップした時に、通常移動やスタック結合より先にマガジン装填を判定します。装填可能なら1件だけ装填ジョブを作り、`inventory_magazine_load_time_per_round` ごとに1発ずつ `Inventory.load_round_into_magazine()` を呼びます。装填中のマガジンには `MagazineLoadIndicator` を重ね、ジョブ全体の進捗を円形に描画します。インベントリを閉じる、または対象マガジンを右クリックするとジョブをキャンセルします。装填中の対象マガジンは、ホバーしても詳細パネルを出さないように抑止します。

マガジンの残弾数と弾薬種は、確認操作なしでインベントリ、Weapon Inspect、HUDへ常時表示します。
...
`AmmoHud` は `PlayerController.magazine_status_changed` を受け取り、画面左下に銃の簡易アイコン、銃名、現在装填中の弾数、所持マガジン数、バックパック内にある対応弾薬数を表示します。選択中銃火器に対応するマガジンアイコンも並べ、装備中マガジンはHUD用の仮エントリとして一覧に含め、マガジン枠全体をオレンジ色の点滅枠で囲みます。マガジンごとの残弾量は常に青いゲージで表示し、空マガジンは赤く表示します。バックパック内にある対応弾薬数が0の場合、HUD上の弾薬数も赤く表示します。フィールド装填中のマガジンには同じ `MagazineLoadIndicator` を重ねます。リロードと送弾の進捗は、照準周辺の円形インジケータで表示します。
