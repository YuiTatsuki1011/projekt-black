...
リロード時はグリッド上の `pistol_magazine` を交換します。`pistol_ammo` はWeapon Inspectから薬室へ1発ずつ装填できます。マガジンを装着しても薬室が空なら即座には発射可能にならず、Rキーまたは射撃入力で送弾動作を行います。マガジンへ弾を詰める操作は、インベントリ内ドラッグ&ドロップとCtrlキーのフィールド装填で扱います。
...
マガジンの残弾数は、`Inventory` の各エントリに保存する `metadata.ammo_count` で管理します。残弾をプレイヤーが確認済みかどうかは `metadata.is_checked` で管理します。床に落とした `DroppedItem` もmetadataを持つため、弾入りマガジンを捨てて拾い直しても残弾数と確認状態は維持されます。
...
`RangedWeaponData` は `magazine_item_id`、`chamber_size`、`starting_spare_magazines`、`chamber_feed_time`、`loaded_magazine_check_time` を持ちます。`PlayerController` は銃火器スロットごとに、装着中マガジン内の弾数、薬室内の弾数、装着マガジン種別、装弾状態が確認済みかどうかを保存します。

Rキーの通常リロードでは、インベントリ内の対応マガジンから `metadata.ammo_count` が最も多いものを選び、現在のマガジンと交換します。現在のマガジンが満タンでも、弾入りの対応マガジンがあれば交換できます。取り外したマガジンは `metadata.ammo_count` を保持したままインベントリへ戻し、空きがなければ `DroppedItem` として床へ落とします。マガジンがない場合、`pistol_ammo` を持っていても直接リロードは行いません。

薬室に弾が残っている状態でマガジン交換した場合は、薬室弾を保持して `マガジン+1` の装弾状態になります。撃ち切って薬室が空の場合は、リロード完了後、または薬室空のままRキー/射撃入力を行った時に、`chamber_feed_time` 秒の送弾動作を開始します。送弾が完了すると弾入りマガジンから薬室へ1発送り、射撃可能状態になります。Inspectからマガジンを装着しただけ、または薬室弾を取り出しただけでは自動送弾しません。

`PlayerController` は、Inspect UIから使うために `insert_active_magazine()`、`eject_active_magazine()`、`insert_chamber_round()`、`eject_chamber_round()` を公開しています。これらは装填状態を更新した後、スロット別の装弾状態を保存し、HUDへ `ammo_changed` と `magazine_status_changed` を送ります。`get_active_magazine_statuses()` はバックパック内マガジンだけでなく、装備中マガジンもHUD用エントリとして返します。
...
`MAGAZINE` スロットからドラッグしたマガジンは、バックパック、外部コンテナ、またはマップへ置けます。`CHAMBER` スロットからドラッグした弾薬も、同じように1発だけ取り出せます。バックパックや外部コンテナ内の対応マガジンを `MAGAZINE` へ落とすと装着し、対応弾薬を `CHAMBER` へ落とすと薬室へ1発だけ直接装填します。
...
## 2026-06-03: Magazine Loading

`Inventory` はマガジン装填用の問い合わせと操作APIを持ちます。`get_magazine_entry_info()` は、対象エントリがマガジンかどうか、対応弾薬ID、容量、現在弾数、確認済み状態を返します。`load_round_into_magazine()` は、別インベントリを含む弾薬エントリから1発だけ消費し、対象マガジンの `metadata.ammo_count` を1増やします。弾を手で詰めたマガジンは、その時点で内容を把握できるため `metadata.is_checked` をtrueにします。対応弾薬でない、満タン、弾薬がない場合は失敗します。

`InventoryScreen` は、弾薬アイテムをマガジン上へドラッグ&ドロップした時に、通常移動やスタック結合より先にマガジン装填を判定します。装填可能なら1件だけ装填ジョブを作り、`inventory_magazine_load_time_per_round` ごとに1発ずつ `Inventory.load_round_into_magazine()` を呼びます。装填中のマガジンには `MagazineLoadIndicator` を重ね、ジョブ全体の進捗を円形に描画します。インベントリを閉じる、または対象マガジンを右クリックするとジョブをキャンセルします。装填中や残弾確認中の対象マガジンは、ホバーしても詳細パネルを出さないように抑止します。

インベントリ内のマガジンは、右クリックメニューの `残弾の確認`、またはマウスホイールクリックで残弾確認ジョブを開始できます。`inventory_magazine_check_time` 経過後、対象マガジンの `metadata.is_checked` をtrueにし、残弾数と弾薬種を表示します。確認中は装填ジョブと同じ `MagazineLoadIndicator` を使います。

`PlayerController` はフィールド装填を担当します。`field_load` 入力、現在選択中の銃火器、要求マガジン、要求弾薬、プレイヤーの `Inventory` を見て、詰められるマガジンを左上から順に選びます。`field_magazine_load_time_per_round` ごとに1発ずつ装填し、満タンになったら次の対応マガジンへ進みます。発砲、リロード、ステップ、サブ武器使用、インベントリ開始時はフィールド装填を中断します。

`AmmoHud` は `PlayerController.magazine_status_changed` を受け取り、選択中銃火器に対応する大きめのマガジンアイコンを画面左下に表示します。装備中マガジンはHUD用の仮エントリとして一覧に含め、マガジン枠全体をオレンジ色の点滅枠で囲みます。フィールド装填中のマガジンには同じ `MagazineLoadIndicator` を重ねます。確認済みマガジンは青い残弾ゲージを維持し、未確認マガジンは `?` を表示します。残弾数と所持総弾数は常時表示せず、R長押しチェック成功時だけ `loaded_magazine_check_finished` を受けて装備中銃火器の残弾数と弾薬種をマガジン列の上に約5秒表示し、その後テキストだけフェードアウトします。リロード、送弾、R長押しチェックの進捗は、照準周辺の円形インジケータで表示します。
