...
- 武器アイテム、または装備中武器の右クリックメニューから `Inspect` を選ぶと、`InventoryScreen` が `WeaponInspectWindow` を動的に生成します。現在は読み取り専用の土台で、仮武器グラフィック、性能バー、パーツスロットを表示します。
...
現在はプレイヤーの `Inventory` ノードで10x8マスのグリッドを管理し、`pistol_ammo` を2x1マス、`pistol_magazine` を1x1マスのアイテムとして配置します。
...
リロード時はグリッド上の `pistol_magazine` を交換します。`pistol_ammo` は今後、マガジンへ弾を詰めるための弾薬として使います。
...
`InventoryScreen` は、武器用コンテキストメニューから `WeaponInspectWindow` を作ります。このウィンドウはシーンファイルではなくコード生成の一時UIです。閉じると破棄され、インベントリを閉じた時やドラッグ開始時にも消します。

現在は読み取り専用です。銃火器では `RangedWeaponData` からダメージ、総装弾数、発射間隔、リロード、反動を読み、下段に `MAGAZINE`、`CHAMBER`、`OPTIC`、`MUZZLE`、`GRIP` などのパーツスロットを表示します。アクティブな銃火器スロットをInspectした場合だけ、現在のマガジン残弾と薬室弾数を表示します。

次に本格モディングへ進む時は、まずパーツスロットをデータ化し、マガジン/チャンバーをこのInspectウィンドウ上のドラッグ&ドロップ対象にします。その後、バックパックとは別にリロード参照用のリグインベントリを追加します。
