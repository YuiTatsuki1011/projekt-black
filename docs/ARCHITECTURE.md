...
- 銃スロットが空になると、プレイヤーは発砲とリロードができず、仮銃の見た目も非表示になります。
- アイテムや装備枠へマウスを乗せた時の詳細パネルも `InventoryScreen` が生成します。武器詳細は `resources/weapons/` のResourceから読み、装備中の同種武器と比較します。
...
詳細パネルは固定ノードではなく、`InventoryScreen` が必要時に `Root` 直下へ作ります。これにより、インベントリ本体のレイアウトと干渉させずに表示位置やサイズを調整できます。
