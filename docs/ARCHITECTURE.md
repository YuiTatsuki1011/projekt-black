...
`InventoryScreen` はプレイヤーインベントリと外部コンテナの右クリックでコンテキストメニューを作ります。スタック可能アイテムでは `Split` から数量入力ダイアログを開き、入力値を `Inventory.split_entry()` へ渡します。Shift+右クリックだけはメニューを省略して半分割を直接実行します。武器アイテムの即装備は、右クリック直実行ではなくメニュー内の `Equip` として扱います。
