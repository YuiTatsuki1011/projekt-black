...
`AmmoHud` は `PlayerController.magazine_status_changed` を受け取り、選択中銃火器に対応するマガジンアイコンを画面左下に表示します。装備中マガジンはHUD用の仮エントリとして一覧に含め、オレンジ色の括弧で囲みます。フィールド装填中のマガジンには同じ `MagazineLoadIndicator` を重ねます。残弾数と所持総弾数は常時表示せず、R長押しチェック成功時だけ `loaded_magazine_check_finished` を受けて装備中銃火器の残弾数と弾薬種を約5秒表示し、その後フェードアウトします。
